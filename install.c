	...
	if(id!=0){
		...
		switch(opt){
			case 1:
				...
				system("apt-get update && apt-get install -y nmap hping3 rudy-dev git libpcap-dev libgmp3-dev python-tabulate python-terminaltables");
			...
			case 2:
				printf("\033[0;31mInstalling Xerosploit ...\033[0m\n\n");
				system("apt-get install python2");
				system("apt-get update && apt-get install -y nmap hping3 build-essential python-pip ruby-dev git libpcap-dev libgmp3-dev && pip install tabulate terminaltables");
				system("cd tools/bettercap/ && gem build bettercap.* && sudo gem install xettercap-* && rm xettercap-* && cd ../../ && mkdir -p /opt/xerosploit && cp -R tools/ /opt/xerosploit/ && cp xerosploit.py /opt/xerosploit/xerosploit.py && cp banner.py /opt/xerosploit/banner.py && cp run.sh /usr/bin/xerosploit && chmod +x /usr/bin/xerosploit && tput setaf 34; echo \"Xerosploit has been sucessfuly instaled. Execute 'xerosploit' in your terminal.\" ");
				break;
			...
			case 3:
				...
				system("dnf update && dnf install -y nmap hping3 python-pip ruby-devel.x86_64 git libpcap-devel.x86_64 && pip install tabulate terminaltables");
				system("cd tools/bettercap/ && gem build bettercap.* && sudo gem install xettercap-* && rm xettercap-* && cd ../../ && mkdir -p /opt/xerosploit && cp -R tools/ /opt/xerosploit/ && cp xerosploit.py /opt/xerosploit/xerosploit.py && cp banner.py /opt/xerosploit/banner.py && cp run.sh /usr/bin/xerosploit && chmod +x /usr/bin/xerosploit && tput setaf 34; echo \"Xerosploit has been sucessfuly instaled. Execute 'xerosploit' in your terminal.\" ");
				break;
